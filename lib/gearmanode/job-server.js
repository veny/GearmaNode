...
JobServer.prototype._processData = function (chunk) {
    ...
    case protocol.PACKET_TYPES.JOB_CREATED:
        // handle
        parsed = protocol.parsePacket(chunk, 'L', 12 + packetLength);
        handle = parsed[1];
    ...
    case protocol.PACKET_TYPES.WORK_COMPLETE:
        ...
        parsed = protocol.parsePacket(chunk, 'nL', 12 + packetLength);
        handle = parsed[1];
    ...
    case protocol.PACKET_TYPES.STATUS_RES:
        ...
        parsed = protocol.parsePacket(chunk, 'nnnnL', 12 + packetLength);
        handle = parsed[1];
        ...
        JobServer.logger.log('verbose', 'STATUS_RES, handle=%s, known=%d, running=%d, num=%d, den=%d',
            handle, result.known, result.running, result.percent_done_num, result.percent_done_den);

        job.emit('status', result); // trigger event
        proceed += packetLength;
        break;
    ...
    case protocol.PACKET_TYPES.WORK_STATUS:
        ...
        parsed = protocol.parsePacket(chunk, 'nnL', 12 + packetLength);
...
console.log('ERROR code=' + parsed[1] + ', msg=' + parsed[2]);
    ...
    if (proceed < chunk.length) {
        this._processData(chunk.slice(proceed));
    }
}
