...
Worker.prototype.response = function (jobServer, packetType, parsedPacket) {
    ...
    case protocol.PACKET_TYPES.JOB_ASSIGN:
        ...
        var rslt = this.functions[parsedPacket[2]];
        console.log('===== ' + rslt()); // TODO parameters + error handling
        this._sendWithJobServer(jobServer, protocol.encodePacket(protocol.PACKET_TYPES.WORK_COMPLETE, null, [handle, rslt()]));
        break;
    }
}


Worker.prototype._sendWithJobServer = function (jobServer, buffer, encoding) {
    var self = this;
    var cb = function (err) {
    ...
    jobServer.send(buffer, encoding, cb);
}
