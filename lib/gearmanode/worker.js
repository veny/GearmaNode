...
Worker.prototype.addFuntion = function(name, callback, options) { // #unit: TODO test it
    var jobServer, errCnt = 0, self = this;
    var canDoCallback = function (err) {
        if (err instanceof Error) {
            errCnt ++;
            Worker.logger.log('error', 'adding function failed, name=%s, server=%s', name, jobServer.getUid());
            // TODO mark the server as failed for future use
            if (errCnt === self.jobServers.length) { // escalate problem as emited 'error' if all servers failed
                Worker.logger.log('error', 'no one job server can be connected');
                self.emit('error', err);
            }
        }
    }
    ...
    for (var i = 0; i < this.jobServers.length; i ++) { // TODO iterate only servers without any previous error
        ...
        jobServer.send(protocol.encodePacket(protocol.PACKET_TYPES.CAN_DO, 'ascii', [name]), canDoCallback);
...
/**
 * Deregisters a given function from job server - worker is no longer
 * able to perform the given function.
 */
Worker.prototype.removeFuntion = function(name) { // #unit: TODO test it
}
...
var workerJobMixin = {
    ...
    workComplete: function (data) {
        ...
        this.clientOrWorker._sendWithJobServer(
            jobServer, protocol.encodePacket(protocol.PACKET_TYPES.PRE_SLEEP));
    },
    /**
