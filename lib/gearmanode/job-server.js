...
JobServer.prototype._processData = function (chunk) {
    ...
    if (chunk.readUInt32BE(0) !== protocol.CONSTANTS.HEADER_RESP) { // \0RES
        // OUT OF SYNC!
        this.mate.unrecoverableError('out of sync with server');
        return;
    }
    ...
    case protocol.PACKET_TYPES.ERROR:
        ...
        this.mate.emit('js_error', 'code=' + parsedPacket[1] + ', msg=' + parsedPacket[2]); // trigger event
        break;
    ...
    case protocol.PACKET_TYPES.JOB_CREATED:
        ...
        if (this.jobsWaiting4Created.length === 0) {
            this.mate.unrecoverableError('empty stack of job waiting 4 packet JOB_CREATED');
            return;
        }
        ...
        this.mate.response(null, packetType, parsedPacket);
        break;
    ...
    case protocol.PACKET_TYPES.JOB_ASSIGN:
        ...
        this.mate.response(this, packetType, parsedPacket);
        break;
