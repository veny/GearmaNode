...
JobServer.prototype.disconnect = function () {
    ...
    if (this.clientOrWorker) {
        this.clientOrWorker.emit('disconnect');
 ...
 *     * success callback response of the job server 
 */
JobServer.prototype.echo = function (data, callback) {
    var self = this;
    var packet, cb;
    if (!common.isString(data)) { return new Error('data not a text'); }
    if (!(callback instanceof Function)) { return new Error('invalid callback (not a function)'); }
    this.echoCallback = callback;

    cb = function (err) {
        if (err instanceof Error) {
            JobServer.logger.log('error', 'failed to send echo, uid=%s', this.getUid());
            delete self.echoCallback;
            callback(err);
        }
    };
...
JobServer.prototype.send = function (data, callback) {
    ...
    if (protocol.CONSTANTS.HEADER_REQ != data.readUInt32BE(0)) {
    ...
    sendCb = function () {
        ...
        JobServer.logger.log('verbose', 'packet sent, type=%s, len=%d',
            protocol.PACKET_CODES[data.readUInt32BE(4)], data.length);
        callback();
    }
...
JobServer.prototype._processData = function (chunk) {
    ...
    case protocol.PACKET_TYPES.ECHO_RES:
        if (this.echoCallback) {
            var cb = this.echoCallback;
            delete this.echoCallback;
            cb(null, parsedPacket[1]);
        } else {
            JobServer.logger.log('warn', 'received ECHO_RES without registered callback');
        }
        break;
