...
JobServer.prototype.processData = function (chunk, callback) {
    ...
    case Job.PACKET_TYPES.JOB_CREATED:
        var jobId = protocol.getJobId(chunk, 12, 12 + packetLength);
    ...
    case Job.PACKET_TYPES.WORK_COMPLETE:
        ...
        var jobId = protocol.getJobId(chunk, 12, next0);
        JobServer.logger.log('verbose', 'work complete, jobId=%s', jobId);
        next0 ++;

        proceed = 1000;
        JobServer.logger.log('debug', 'Result>>>> %s', chunk.toString('utf8', next0));
console.log("ss " + next0);
        break;
    default:
