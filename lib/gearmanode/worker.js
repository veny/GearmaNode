 ...
 * @param {boolean} options.withUnique TODO: flag whether a job will be grabbed with the client assigned unique ID
 */
 ...
 * @param {number} options.timeout timeout value, the job server will mark the job as failed and notify any listening clients (optional) TODO
 * @param {string} options.toStringEncoding if given received payload will be converted to String with this encoding, otherwise payload turned over as Buffer TODO
...
common.mixin({
    ...
    _sendAndClose: function (packetType, packetData) {
        var jobServer = this.clientOrWorker._getJobServerByUid(this.jobServerUid);
        jobServer.send(protocol.encodePacket(packetType, null, packetData));
