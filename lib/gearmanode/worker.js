...
var workerJobMixin = {
    ...
    workComplete: function (data) {
        ...
        this.clientOrWorker._sendWithJobServer(
            jobServer, protocol.encodePacket(protocol.PACKET_TYPES.WORK_COMPLETE, null, [this.handle, data.toString()]));
    ...
    reportError: function () {
        Worker.logger.log('warn', 'work fail, handle=%s', this.handle);
        var jobServer = this.clientOrWorker._getJobServerByUid(this.jobServerUid);
        this.clientOrWorker._sendWithJobServer(
            jobServer, protocol.encodePacket(protocol.PACKET_TYPES.WORK_FAIL, null, [this.handle]));
        this.clientOrWorker._preSleep(jobServer);
        this.close();
    }
}
