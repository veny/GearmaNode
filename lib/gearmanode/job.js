...
Job.prototype.encodeGS = function () { // #unit: TODO test it
    var packetLength = this.id.length
    var buff = new Buffer(12 + packetLength); // 12 byte header

    buff.writeUInt32BE(0x00524551, 0); // \0REQ
    buff.writeUInt32BE(Job.PACKET_TYPES.GET_STATUS, 4);
    buff.writeUInt32BE(packetLength, 8);

    var offset = 12;
    buff.write(this.id, 12);

    Job.logger.log('verbose', 'job encoded to be sent, type=GET_STATUS, buffer.size=%d', buff.length);
...
Job.prototype.getStatus = function () {
    if (!this.background) {
        return new Error('this is not a background job, no status');
    }
    if (!this.jobServer) {
        return new Error('no jobServer (never ever submited job?)');
    }
    ...
    this.jobServer.socket.write(this.encodeGS(), 'ascii');
}
