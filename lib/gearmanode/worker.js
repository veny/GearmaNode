...
Worker.prototype.response = function (jobServer, packetType, parsedPacket) {
    ...
    case protocol.PACKET_TYPES.JOB_ASSIGN:
        ...
        var job = new Job(this, {handle: parsedPacket[1], name: parsedPacket[2], payload: parsedPacket[3],
            worker: this, jobServerUid: jobServer.getUid()});
        var func = this.functions[job.name];
        console.log('===== ' + job.toString()); // TODO parameters + error handling
job.sendWorkComplete();
        this._sendWithJobServer(jobServer, protocol.encodePacket(protocol.PACKET_TYPES.WORK_COMPLETE, null, [job.handle, func(job)]));
        this._sendWithJobServer(jobServer, protocol.encodePacket(protocol.PACKET_TYPES.PRE_SLEEP));
        break;
    }
}
...
var workerJobMixin = {
    /**
     * Sends a notification to the server (and any listening clients) the that job completed successfully.
     */
    sendWorkComplete: function (data) {
        var jobServer = this.clientOrWorker._getJobServerByUid(this.jobServerUid);
        //
        //jobServer.send(buffer, function() {});
        console.log('=============XXXXXXXXXXXXx ' + this.toString());
    }
}
