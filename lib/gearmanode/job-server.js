...
JobServer.prototype.connect = function (callback) {
    ...
    if (this.connected) {
        ...
        this.socket.on('error', function (err) {
            ...
            // ECONNREFUSED emits special event which 
            self.clientOrWorker.emit((err.code == 'ECONNREFUSED' && self.clientOrWorker._type === 'Client') ? 'js_econnrefused' : 'error', err); // trigger event
            self.disconnect();
