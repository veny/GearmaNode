...
JobServer.prototype._processData = function (chunk) {
    ...
    var dataSize = chunk.readUInt32BE(8);

    // store chunk if data is segmented into more packets to be concatenated later
    if (chunk.length < (protocol.CONSTANTS.HEADER_LEN + dataSize)) {
        JobServer.logger.log('verbose', 'segmented packet found, expectedDataSize=%d, packetLen=%d', dataSize, chunk.length);
