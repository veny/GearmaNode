...
JobServer.prototype.processData = function (chunk, callback) {
    ...
    case Job.PACKET_TYPES.STATUS_RES:
        ...
        jobId = Job.getJobId(chunk, 12, next0);
        next0 ++;
        var knownStatus = String.fromCharCode(chunk.readUInt8(next0));
console.log('next0 ' + (typeof knownStatus));
        next0 += 2;
        var runningStatus = String.fromCharCode(chunk.readUInt8(next0));
        next0 += 2;
        var percent = String.fromCharCode(chunk.readUInt8(next0));
console.log('STATUS ' + jobId + ', knownStatus=' + knownStatus + ", runningStatus=" + runningStatus + ', percent=' + percent);
        break;
 ...
 */
JobServer.prototype.getId = function() {
